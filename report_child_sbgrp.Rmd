...
Q(df = `r params$sbgrp_k - params$sbgrp_Qdf`) = `r round(params$sbgrp_QE, 4)`, _p_ `r if(params$sbgrp_QEp < 0.0001){paste("< .0001")} else {paste("= ", format(round(params$sbgrp_QEp, 4), scientific = FALSE))}`
...
QM(df = `r params$sbgrp_Qdf - 1`) = `r round(params$sbgrp_QM, 4)`, _p_ `r if(params$sbgrp_QMp < 0.0001){paste("< .0001")} else {paste("= ", format(round(params$sbgrp_QMp, 4), scientific = FALSE))}`.

### With Knapp-Hartung-Adjustment: 

F (df1 = `r params$sbgrp_Qdf`, df2 = `r params$sbgrp_k - params$sbgrp_Qdf`) = `r round(params$sbgrp_QM_knha, 4)`, _p_`r if(params$sbgrp_QMp_knha < 0.0001){paste("< .0001")} else {paste("= ", format(round(params$sbgrp_QMp_knha, 4), scientific = FALSE))}`.
